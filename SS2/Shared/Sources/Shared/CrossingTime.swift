...
public struct CrossingTime: Equatable {
    public let date: Date
    public let timeDiff: TimeDiff
    ...
    public init(date: Date, timeDiff: TimeDiff) {
        self.date = date
        self.timeDiff = timeDiff
    }
    
    public static func == (lhs: CrossingTime, rhs: CrossingTime) -> Bool {
        return lhs.date == rhs.date && lhs.timeDiff == rhs.timeDiff
    }
}

public enum TimeDiff: Equatable, Codable, Hashable {
    case combined(hours: Int, minutes: Int)
    
    // Default values as per business logic
    public static let defaultFirst = TimeDiff.combined(hours: 0, minutes: 0) // "0h0m"
    public static let defaultSecond = TimeDiff.combined(hours: 2, minutes: 0) // "2h0m"
    ...
    public static func from(date: Date, relativeTo baseDate: Date) -> TimeDiff {
